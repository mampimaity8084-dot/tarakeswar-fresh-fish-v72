...
function addProductStudio(){
 ...
 $('v6AnalyzeProduct').onclick=async()=>{const f=file.files?.[0];if(!f){result.style.display='block';result.textContent='📷 আগে product photo দিন।';return}if(f.size>6*1024*1024){result.style.display='block';result.textContent='ছবিটি 6 MB-এর মধ্যে দিন।';return}result.style.display='block';result.textContent='⏳ AI product analysis চলছে...';try{const dataUrl=await new Promise((res,rej)=>{const r=new FileReader();r.onload=()=>res(r.result);r.onerror=rej;r.readAsDataURL(f)});draft=await post('/.netlify/functions/ai-product-studio',{image_data_url:dataUrl,hint:$('v6ProductHint').value.trim(),price_hint:$('v6PriceHint').value.trim(),stock_hint:$('v6StockHint').value.trim()});const p=draft.product||{};$('v6Name').value=p.name||'';$('v6Category').value=p.category||'Fish';$('v6Price').value=p.price??'';$('v6Unit').value=p.unit||'KG';$('v6Stock').value=p.stock_qty??'';$('v6Tags').value=Array.isArray(p.tags)?p.tags.join(', '):(p.tags||'');$('v6Description').value=p.description||'';$('v6Prep').value=p.preparation||'';fields.style.display='grid';save.disabled=false;result.innerHTML='✅ Draft তৈরি হয়েছে। <b>Price/stock/description দেখে নিজে approve করুন</b> তারপর Save চাপুন.'}catch(e){result.textContent='❌ '+e.message}};
 save.onclick=async()=>{if(!draft)return;try{save.disabled=true;result.textContent='⏳ Product save হচ্ছে...';const p={name:$('v6Name').value.trim(),category:$('v6Category').value.trim()||'Fish',price:Number($('v6Price').value||0),unit:$('v6Unit').value.trim()||'KG',stock_qty:Number($('v6Stock').value||0),description:$('v6Description').value.trim(),tags:$('v6Tags').value.split(',').map(x=>x.trim()).filter(Boolean),preparation:$('v6Prep').value.trim()};if(!p.name||!p.price){throw Error('Name এবং valid price দিন')}const data=await post('/.netlify/functions/ai-product-studio',{action:'save',product:p,image_data_url:draft.image_data_url||null});result.textContent='✅ Product saved. Customer app refresh হলে নতুন product দেখা যাবে।';save.disabled=true}catch(e){save.disabled=false;result.textContent='❌ '+e.message}};
...
function enhanceCustomerAI(){
 ...
 window.tffV6AskAI=async(q)=>{const products=(window.tffProducts||[]).slice(0,120).map(p=>({id:p.id,name:p.name,category:p.category,price:p.price,unit:p.unit,available:p.available!==false,stock_qty:p.stock_qty??null,description:p.description||''}));const r=await fetch('/.netlify/functions/ai-assistant',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:q,context:{products,cart:(window.tffCart||[]).map(x=>({id:x.id,name:x.name,price:x.price})),profile:JSON.parse(localStorage.getItem('tff_profile')||'{}')}})});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||'AI unavailable');return d};
