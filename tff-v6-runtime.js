  ...
  if (page === 'customer') {
    window.uploadCustomerPhoto = async function () {
      ...
      try {
        ...
        const response = await fetch((window.TFF_API_BASE || '') + '/.netlify/functions/customer-profile', {
          method: 'POST',
...
(function(){'use strict';
 ...
 async function askServer(q){const ps=(window.products||[]).slice(0,80).map(p=>({id:p.id,name:p.name,category:p.category,price:p.price,unit:p.unit,available:p.available!==false,description:p.description||''}));const profile=JSON.parse(localStorage.getItem('tff_profile')||'{}');const r=await fetch('/.netlify/functions/ai-assistant',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:q,context:{products:ps,profile,cart:(window.cart||[]).slice(0,20)}})});const d=await r.json().catch(()=>({}));if(!r.ok||!d.text)throw Error(d.error||'AI unavailable');return d.text}
...
(function(){'use strict';if(!location.pathname.toLowerCase().includes('admin.html'))return;function esc(s){return String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}async function boot(){const host=document.querySelector('.wrap');if(!host||document.getElementById('tffV53Integrations'))return;const sec=document.createElement('section');sec.id='tffV53Integrations';sec.className='panel';sec.innerHTML=`<div class="panel"><h2>🚀 V5.3 AI • WhatsApp • Backup Center</h2><p class="muted">এখান থেকে integration-এর status ও test চালাতে পারবেন। Secret key এখানে লেখা হবে না—শুধু Netlify Environment Variables-এ থাকবে।</p><div class="row"><div><label>WhatsApp Group ID (optional)</label><input id="v53Group" placeholder="Meta Groups API-এর Group ID"></div><div><label>Group Test Message</label><input id="v53Msg" value="🐟 Tarakeswar Fresh Fish — আজকের fresh stock available!"></div></div><div class="actions"><button class="btn green" id="v53Send">📤 Send Group Test</button><button class="btn light" id="v53Backup">💾 Download Full Backup</button><label class="btn light">♻️ Restore Backup<input id="v53Restore" type="file" accept="application/json" style="display:none"></label></div><div id="v53MsgOut" class="muted" style="margin-top:8px"></div></div>`;host.appendChild(sec);const out=sec.querySelector('#v53MsgOut');sec.querySelector('#v53Backup').onclick=()=>location.href='/.netlify/functions/backup-export';sec.querySelector('#v53Send').onclick=async()=>{try{const {data:{session}}=await sb.auth.getSession();if(!session)throw Error('Admin login required');const group=sec.querySelector('#v53Group').value.trim();if(!group)throw Error('Group ID দিন');const text=sec.querySelector('#v53Msg').value.trim();const r=await fetch('/.netlify/functions/send-whatsapp',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+session.access_token},body:JSON.stringify({group_id:group,text})});const d=await r.json();if(!r.ok||!d.ok)throw Error(d.error||'Group send failed');out.textContent='✅ WhatsApp group message sent.'}catch(e){out.textContent='❌ '+e.message}};sec.querySelector('#v53Restore').onchange=async e=>{const f=e.target.files?.[0];if(!f)return;try{const {data:{session}}=await sb.auth.getSession();if(!session)throw Error('Admin login required');const payload=JSON.parse(await f.text());const r=await fetch('/.netlify/functions/restore-backup',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+session.access_token},body:JSON.stringify(payload)});const d=await r.json();if(!r.ok||!d.ok)throw Error(d.error||'Restore failed');out.textContent='✅ Restore complete. Page reload করুন।'}catch(err){out.textContent='❌ '+err.message}}}
